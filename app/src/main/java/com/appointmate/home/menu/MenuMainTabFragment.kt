...
import com.appointmate.home.barber.BarberFragment
import com.appointmate.home.barber.BarberFragmentDirections
...
class MenuMainTabFragment : Fragment() {
    ...
    private val adapter: MenuAdapter = MenuAdapter {
        when (MenuItemEnum.from(it)) {
            ...
            MenuItemEnum.BARBER -> {
                findNavController().navigateSafe(BarberFragmentDirections.actionGlobalBarberFragment())
            }
            else -> {}
        }
    ...
    private fun setupViews() {
        ...
        adapter.updateData(arrayListOf("settings", "administrator","barber"))
    }
}